...
def cartesianPlot(window_title="Cartesian plot", points=None, lines=None, arrows=None, loops=None,
    ...
    def drawaxes(qp):
        minx, miny, maxx, maxy = plot_range
        gtt = GT.twopoints
        pen = QtGui.QPen(QtGui.QColor(84,84,84), 2, QtCore.Qt.SolidLine)
        qp.setPen(pen)
        #sccl = canvas.create_line
        a,b,c,d=gtt(0, miny, 0, maxy)
        qp.drawLine(a,b,c,d)
        a,b,c,d=gtt(minx, 0, maxx, 0)
        qp.drawLine(a,b,c,d)
    ...
    def drawpoints(qp):
        ...
        if pointLabels:
            ...
            for i,(x,y) in enumerate(points):
                ...
                pen = QtGui.QPen(QtGui.QColor(255,255,255), 1, QtCore.Qt.SolidLine)
                brush = QtGui.QBrush(QtGui.QColor(color))
                qp.setPen(pen)
                qp.setBrush(brush)
                print ps
                qp.drawEllipse(x1-ps, y1-ps, 2*ps, 2*ps)
                ...
                if i < len_pointLabels: # Show point label
                    ...
                    pen = QtGui.QPen(QtGui.QColor(139,0,139), 1, QtCore.Qt.SolidLine)
                    qp.setPen(pen)
                    qp.drawText(x1+9,y1-6,label)
    ...
    def drawarrows(qp):
        ...
        for i,((x1,y1),(x2,y2)) in enumerate(arrows):
            ...
            x1,y1,x2,y2=gtt(x1, y1, x2, y2)
            p0=(x2 - (x2-x1)/20, y2 -(y2-y1)/20)
            vd= ((y1-y2)/30,(x2-x1)/30)
            p1=(p0[0]+vd[0],p0[1]+vd[1])
            p2=(p0[0]-vd[0],p0[1]-vd[1])
            pen = QtGui.QPen(QtGui.QColor(color), 1, QtCore.Qt.SolidLine)
            qp.setPen(pen)
            qp.drawLine(x1,y1,x2,y2)
            qp.drawLine(p1[0],p1[1],x2,y2)
            qp.drawLine(p2[0],p2[1],x2,y2)
    ...
    def drawlines(qp):
        ...
        for i,((x1,y1),(x2,y2)) in enumerate(lines):
            ...
            pen = QtGui.QPen(QtGui.QColor(color), w, QtCore.Qt.SolidLine)
            qp.setPen(pen)
            a,b,c,d=gtt(x1, y1, x2, y2)
            qp.drawLine(a,b,c,d)
    ...
    def drawloops(qp):
        ...
        for i,(x,y) in enumerate(loops):
            ...
            qp.setPen(pen)
            qp.drawEllipse(x1-ps2,y1-ps3, 2*ps2, ps3)
            qp.drawLine(x1+ps, y1, x1, y1+ps)
    ...
    def drawGraph(e):
        ...
        qp = QtGui.QPainter()
        qp.begin(master)
        width = float(master.width())
        height = float(master.height())
        viewport = (pad, pad, width-pad, height-pad)
        GT = Transformer(plot_range, viewport)
        drawbackground(qp)
        if grid_spacing: drawgrid(qp)
        if axes: drawaxes(qp)
        if arrows: drawarrows(qp)
        if lines: drawlines(qp)
        if loops: drawloops(qp)
        if points: drawpoints(qp)
        qp.end()
    ...
    print window_title
    master.setWindowTitle( str(window_title) )
    WIDTH = HEIGHT = 500
    master.setGeometry(300, 300, WIDTH, HEIGHT)
    master.show()
...
if __name__=='__main__': # Demo -----------------------------------
    ...
    if 1:
        ...
        print "heyheo"
        m=cartesianPlot(window_title="Graph2", points=p, lines=l, pointLabels=pl, pointW=ps,
